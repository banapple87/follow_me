...
def haversine_distance(lat1, lon1, floor1, lat2, lon2, floor2):
    ...
    vertical_distance = abs(floor2 - floor1) * 0.005  # 0.005 km = 5m

    # Pythagoras를 이용해 총 거리 계산
    return math.sqrt(horizontal_distance ** 2 + vertical_distance ** 2)

# Intermediate points 생성 함수
def generate_intermediate_points(p1, p2, max_points=10, min_points=10, min_distance=0.005, max_distance=0.05):
    """
...
def optimal_path():
    ...
    # 시작 좌표를 가장 가까운 그래프 노드로 매핑
    start_node = min(dense_path_coordinates, key=lambda node: haversine_distance(
        start_point[1], start_point[2], start_point[0], node[1], node[2], node[0]
    ))

    # 브랜드 좌표를 가장 가까운 그래프 노드로 매핑
    target_nodes = [
        min(dense_path_coordinates, key=lambda node: haversine_distance(
